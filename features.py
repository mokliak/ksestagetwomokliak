...
# ── Russian launch-site regions (lat, lon) ──────────────────────────────────
LAUNCH_SITE_COORDS = {
    "Krasnodar Krai":  (45.04, 38.98),
    "Kursk Oblast":    (51.73, 36.19),
    "Oryol Oblast":    (52.97, 36.07),
    "Bryansk Oblast":  (53.24, 34.36),
    "Rostov Oblast":   (47.22, 39.72),
}
...
def load_raw(path: str | Path) -> pd.DataFrame:
    ...
    # Restrict to 2025 onward — older years are noisy / less relevant
    df = df[df["started_at"].dt.year >= 2025]
    ...
    # Merge "Kyiv City" and "Kyivska oblast" into one combined entry —
    # avoids splitting the same geographic signal into two sparse rows
    df["oblast"] = df["oblast"].replace({
        "Kyiv City": "Kyiv and Kyiv oblast",
        "Kyivska oblast": "Kyiv and Kyiv oblast",
    })
...
def build_daily_panel(df: pd.DataFrame) -> pd.DataFrame:
    ...
    level_counts = level_counts.rename(columns={
        "level_raion":   "raion_alerts",
        "level_hromada": "hromada_alerts",
        "level_oblast":  "oblast_alerts",
    })
...
def _fetch_weather(site: str, start: str, end: str) -> pd.DataFrame:
    coords = LAUNCH_SITE_COORDS.get(site)
    if not coords:
        return pd.DataFrame()
    lat, lon = coords
    try:
        ...
        print(f"    Weather fetch failed ({site}): {e}")
        return pd.DataFrame()
...
def add_weather_features(
    ...
    if cache_path and cache_path.exists():
        ...
        frames = []
        sites = list(LAUNCH_SITE_COORDS.keys())
        for i, site in enumerate(sites):
            print(f"  Fetching weather {i + 1}/{len(sites)}: {site}", end="\r")
            wdf = _fetch_weather(site, start, end)
            if not wdf.empty:
                frames.append(wdf)
            time.sleep(0.35)
        print()
    ...
    # Average conditions across all launch sites per day (national-level signal,
    # since any one site going clear can enable a launch)
    daily = (
        ...
        .agg(
            ...
            high_wind      = ("high_wind", "max"),  # any site too windy to launch
        )
        .reset_index()
    )
...
def _score_with_gemini(date_str: str, headlines: list[str], query_type: str) -> float:
    """
    Send headlines to Gemini for 0-1 scoring.
...
def add_news_features(
    ...
    if cache_path and cache_path.exists():
        ...
        cutoff = max(all_dates) - datetime.timedelta(days=60)
        unique_dates = [d for d in all_dates if d >= cutoff][:40]
        rows = []
        for i, d in enumerate(unique_dates):
            ...
            for qt, query in NEWS_QUERIES.items():
                    ...
                    row[qt]   = _score_with_gemini(date_str, headlines, qt)
...
def add_geopolitical_features(
    ...
    massive_dates = sorted(nat[nat["is_massive"] == 1]["date"].tolist())

    def days_since(d):
        idx = bisect.bisect_left(massive_dates, d)
        if idx == 0:
            return np.nan
        return (d - massive_dates[idx - 1]).days
...
def add_dummy_variables(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    CENTRAL_OBLASTS = {
            ...
            "Kyiv and Kyiv oblast","Chernihivska oblast","Sumska oblast",
        }
...
# ════════════════════════════════════════════════════════════════════════════
# 8b. REGION-TO-REGION RISK COMPARISON
# ════════════════════════════════════════════════════════════════════════════

def build_region_risk_ranking(panel: pd.DataFrame) -> pd.DataFrame:
    """
    Rank oblasts by overall risk using alert frequency and severity.
    Flags top third as 'high_risk', bottom third as 'low_risk'.
    """
    summary = (
        panel.groupby("oblast")
        .agg(
            total_alerts   = ("alert_count", "sum"),
            avg_daily      = ("alert_count", "mean"),
            days_with_alert= ("had_alert", "sum"),
            total_days     = ("had_alert", "count"),
            avg_duration    = ("avg_dur_min", "mean"),
        )
        .reset_index()
    )
    summary["alert_rate"] = summary["days_with_alert"] / summary["total_days"]

    # Composite risk score: normalised blend of frequency + rate + duration
    for col in ["total_alerts", "alert_rate", "avg_duration"]:
        rng = summary[col].max() - summary[col].min()
        summary[f"{col}_norm"] = (
            (summary[col] - summary[col].min()) / rng if rng > 0 else 0.0
        )
    summary["risk_score"] = (
        summary["total_alerts_norm"] * 0.4
        + summary["alert_rate_norm"] * 0.4
        + summary["avg_duration_norm"] * 0.2
    )

    summary = summary.sort_values("risk_score", ascending=False).reset_index(drop=True)

    n = len(summary)
    summary["risk_tier"] = "medium_risk"
    summary.loc[: n // 3 - 1, "risk_tier"] = "high_risk"
    summary.loc[n - n // 3 :, "risk_tier"] = "low_risk"
...
def build_feature_set(
    ...
    print("[8/8] Building region risk ranking...")
    region_risk = build_region_risk_ranking(panel)
    region_risk_f = data_dir / "region_risk.csv"
    region_risk.to_csv(region_risk_f, index=False)

    print(f"  Region risk ranking saved → {region_risk_f}")
    print(region_risk[["oblast", "risk_score", "risk_tier"]].to_string(index=False))
